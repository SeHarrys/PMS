			...
			case strings.Index(cmd, "MAIL FROM:") == 0:
				if len(cmd) > 10 {
					...
					if dmn.Plugins["spf"] == true {
						SPF := Spf{helo: cl.helo, client_ip: cl.ip, envelope: cl.mail_from}
						SPF.New()
...
func (dmn *Daemon) banHost(ip string) {
	if dmn.bannedHosts[ip] == Config.Smtp.Banlimit {
		dmn.bannedHosts[ip] = int(time.Now().Unix()) + Config.Smtp.Bantime
	} else {
		dmn.bannedHosts[ip] = dmn.bannedHosts[ip] + 1
	}
}
