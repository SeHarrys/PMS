...
// FIXME
func (dmn *Daemon) readData(client *Client) (input string, err error) {
	var reply string
	var msg string
	suffix := "\r\n.\r\n"

	for err == nil {
		client.conn.SetDeadline(time.Now().Add(dmn.timeout * time.Second))
	...
	if err != nil {
		...
		dmn.killClient(client, "500 Failed parsing message")
		return
	}
...
func (dmn *Daemon) readSmtp(client *Client) (input string, err error) {
	var reply string
	suffix := "\r\n"
